# Debian 12 basic test - no ISO modification
# Tests: VM creation, boot command, preseed, SSH, provisioning

source "vcd-iso" "debian-basic" {
  # VCD Connection
  host                = var.vcd_host
  ...
  # VM Configuration
  vm_name       = "${var.vm_name}-debian-basic"
  guest_os_type = "debian12_64Guest"
  CPUs          = 2
  memory        = 2048
  disk_size_mb  = 20480

  # Network - uses auto_discover_ip with retry logic for IP conflicts
...
build {
  sources = ["source.vcd-iso.debian-basic"]

  provisioner "shell" {
    inline = [
      "echo '=== Debian Basic Test ==='",
