...
func (s *StepDiscoverIP) Run(ctx context.Context, state multistep.StateBag) multistep.StepAction {
	...
	if s.ManualIP != "" {
		...
		// Also store gateway and DNS if provided (for cd_content templates)
	...
	if dRaw == nil {
		state.Put("error", fmt.Errorf("driver not found in state - connection may have failed"))
		return multistep.ActionHalt
	}
	...
	if err != nil {
		state.Put("error", fmt.Errorf("failed to get VDC %s: %w", s.VDCName, err))
		return multistep.ActionHalt
	}
