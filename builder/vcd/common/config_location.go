...
type LocationConfig struct {
	...
	// Gateway address for the VM. When auto_discover_ip is true, overrides the
	// discovered gateway. When vm_ip is set (manual IP), provides gateway for
	// cd_content template variables ({{ .VMGateway }}).
	VMGateway string `mapstructure:"vm_gateway"`
	// DNS server for the VM. When auto_discover_ip is true, overrides the
	// discovered DNS. When vm_ip is set (manual IP), provides DNS for
	// cd_content template variables ({{ .VMDNS }}).
...
func (c *LocationConfig) Prepare() []error {
	...
	// Validate vm_gateway and vm_dns are only used with auto_discover_ip or manual IP
	if !c.AutoDiscoverIP && c.VMIPAddress == "" && (c.VMGateway != "" || c.VMDNS != "") {
		errs = append(errs, fmt.Errorf("'vm_gateway' and 'vm_dns' require either 'auto_discover_ip' or 'vm_ip' to be set"))
	}
